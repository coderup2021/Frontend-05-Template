...
export default class Carousel extends Component {
	...
	render() {
		...
		this.root.addEventListener("pan", (event) => {
			...
			console.log(this[STATE].position, current, x, event.clientX, event.startX, ax);
		...
		this.root.addEventListener("end", (event) => {
			...
			const x = event.clientX - event.startX - ax;
			const current = this[STATE].position - ((x - (x % this.WIDTH)) % this.WIDTH);
			...
			for (let offset of [0, Math.sign(x - 250 * Math.sign(x))]) {
				...
				this[STATE].position =
					this[STATE].position - (x - (x % this.WIDTH)) / this.WIDTH - direction;
				this[STATE].position =
					((this[STATE].position % children.length) + children.length) % children.length;
			}
		});
